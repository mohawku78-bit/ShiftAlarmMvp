...
$watchSignalVibrator = Read-RepoFile "wear\src\main\java\com\example\shiftalarmmvp\wear\WatchAlarmSignalVibrator.kt"
...
Assert-Contains "run-watch-side-by-side-smoke.ps1" $smokeScript 'Assert-LastExitCode "assert-watch-smoke-result.ps1"'
...
Assert-Contains "assert-watch-smoke-result.ps1" $smokeAssertScript 'phone received expected watch control'
Assert-Contains "assert-watch-smoke-result.ps1" $smokeAssertScript 'phone accepted expected watch control'
Assert-Contains "assert-watch-smoke-result.ps1" $smokeAssertScript 'watch received expected phone control ack'
...
Assert-Contains "AlarmVibrationPatternsTest.kt" $alarmVibrationPatternsTest 'watchBridgeFallbackNotificationUsesSoftMultiPulsePattern'
Assert-Contains "AlarmVibrationPatternsTest.kt" $alarmVibrationPatternsTest 'minPulseCount = 10'
Assert-Contains "AlarmVibrationPatternsTest.kt" $alarmVibrationPatternsTest 'minTotalDurationMillis = 14_000L'
Assert-Contains "AlarmRingingService.kt" $ringingService 'watch_alarm_bridge_channel_v6'
...
Assert-Contains "WatchAlarmListenerService.kt" $watchListener 'WatchAlarmSignalVibrator.startOneShot(this, payload)'
...
Assert-Contains "WatchAlarmListenerService.kt" $watchListener 'WatchAlarmSignalVibrator.cancel()'
...
Assert-Contains "WatchAlarmNotifier.kt" $watchNotifier 'shift_alarm_watch_alarm_v8'
Assert-Contains "WatchAlarmNotifier.kt" $watchNotifier 'shift_alarm_watch_alarm_v7'
Assert-Contains "WatchAlarmNotifier.kt" $watchNotifier 'shift_alarm_watch_alarm_v6'
...
Assert-Contains "WatchVibrationPatterns.kt" $watchVibrationPatterns 'NOTIFICATION_RAMP_AMPLITUDES'
...
Assert-Contains "WatchVibrationPatternsTest.kt" $watchVibrationPatternsTest 'notificationVibrationUsesSoftMultiPulseRamp'
Assert-Contains "WatchVibrationPatternsTest.kt" $watchVibrationPatternsTest 'minPulseCount = 10'
Assert-Contains "WatchVibrationPatternsTest.kt" $watchVibrationPatternsTest 'minTotalDurationMillis = 14_000L'
...
Assert-Contains "WatchAlarmSignalVibrator.kt" $watchSignalVibrator 'VibrationEffect.createWaveform'
Assert-Contains "WatchAlarmSignalVibrator.kt" $watchSignalVibrator 'WatchVibrationPatterns.NOTIFICATION_RAMP_PATTERN'
Assert-Contains "WatchAlarmSignalVibrator.kt" $watchSignalVibrator 'WatchVibrationPatterns.NOTIFICATION_RAMP_AMPLITUDES'
Assert-Contains "WatchAlarmSignalVibrator.kt" $watchSignalVibrator '-1'
Assert-Contains "WatchAlarmNotifier.kt" $watchNotifier 'enableVibration(false)'
...
Assert-Contains "verify-watch-side-by-side.ps1" $verifyScript '-Mode stop -Clear -Assert'
Assert-Contains "verify-watch-side-by-side.ps1" $verifyScript '-Mode snooze -Clear -Assert'
Assert-Contains "run-watch-side-by-side-smoke.ps1" $smokeScript '[ValidateSet("preview", "control", "orphan", "stop", "snooze", "cleanup")]'
...
Assert-Contains "run-watch-side-by-side-smoke.ps1" $smokeScript '$effectiveAutoWatchAction = $Mode'
Assert-Contains "run-watch-side-by-side-smoke.ps1" $smokeScript '-Mode $Mode always tests the matching watch action'
Assert-Contains "run-watch-side-by-side-smoke.ps1" $smokeScript 'cleanup = "com.example.shiftalarmmvp.action.WATCH_CONTROL_TEST_STOP"'
...
Assert-Contains "assert-watch-smoke-result.ps1" $smokeAssertScript '[ValidateSet("preview", "control", "orphan", "stop", "snooze")]'
Assert-Contains "assert-watch-smoke-result.ps1" $smokeAssertScript '$AssertionMode = if ($Mode -in @("stop", "snooze")) { "control" } else { $Mode }'
Assert-Contains "assert-watch-smoke-result.ps1" $smokeAssertScript '$ResolvedExpectedAction'
...
Assert-Contains "assert-watch-smoke-result.ps1" $smokeAssertScript 'start one-shot alarm vibration'
Assert-Contains "assert-watch-smoke-result.ps1" $smokeAssertScript 'phone received orphan expected watch control'
Assert-Contains "assert-watch-smoke-result.ps1" $smokeAssertScript 'phone rejected orphan expected watch control as stale'
...
Assert-Contains "run-watch-full-validation.ps1" $fullValidationScript 'SkipUnitTests'
Assert-Contains "run-watch-full-validation.ps1" $fullValidationScript 'Watch unit tests'
Assert-Contains "run-watch-full-validation.ps1" $fullValidationScript 'com.example.shiftalarmmvp.watch.*'
Assert-Contains "run-watch-full-validation.ps1" $fullValidationScript 'com.example.shiftalarmmvp.wear.*'
...
Assert-Contains "run-watch-full-validation.ps1" $fullValidationScript '$mode = if ($Scenario -eq "orphan") { "orphan" } else { $Scenario }'
