...
- 정확 알람/알림/배터리 예외 안내가 홈과 편집 화면에서 같은 우선순위 규칙으로 연결됩니다.
- 홈 배너 버전: `v063`
...
- [ ] Exact Alarm 권한 허용/거부/재진입 시나리오와 다음 단계 안내 점검
...
## 작업 기록
- 2026-03-11 `v063`: 정확 알람/알림/배터리 예외 상태를 공통 온보딩 모델로 묶어 홈 신뢰도 패널과 편집 화면이 같은 우선순위와 문구를 사용하도록 정리. 설정 복귀 후 토스트도 다음 단계 안내를 이어서 보여주게 수정. 확인 항목: `:app:compileDebugKotlin -x kspDebugKotlin`, `:app:testDebugUnitTest`, `:app:assembleDebug`. 남은 리스크: 실기기에서 권한 화면 복귀 후 토스트와 제조사 배터리 경로 체감 확인 필요.
